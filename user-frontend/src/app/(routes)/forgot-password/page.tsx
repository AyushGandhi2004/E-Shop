...
const ForgotPassword = () => {
    ...
    const reqOtpMutation = useMutation({
    ...
    const verifyOtpMutation = useMutation({
        mutationFn: async () => {
            if (!email) return;
            const response = await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/api/verify-forgot-password-otp`, {
    ...
    const resetPasswordMutation = useMutation({
        mutationFn: async ({ password }: { password: string }) => {
            if (!password) return;
            const response = await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/api/reset-password`, { newPassword: password, email: email });
        ...
        onError: (error: AxiosError) => {
            const errorMessage = (error.response?.data as { message?: string })?.message || "Invalid New Password, Please try again.";
            setServerError(errorMessage);
        }
    })

    const onSubmitEmail = async ({ email }: { email: string }) => {
        reqOtpMutation.mutate({ email })
    }
                        ...
                        step === "email" && (
                            <>
                                ...
                                <form onSubmit={handleSubmit(onSubmitEmail)}>
                                    ...
                                    <button
                                        type="submit"
                                        disabled={reqOtpMutation.isPending}
                                        className="bg-blue-500 text-white py-2 px-4 rounded-md hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500 w-full mt-2"
                                    >
                                        {reqOtpMutation.isPending ? "Sending OTP..." : "Send OTP"}
                                    </button>
                                    {
                                        serverError && (
                        ...
                        step === "otp" && (
                            <div className="">
                                ...
                                <p className="text-center text-sm mt-4">
                                    {
                                        canResend ? (
                                            <button onClick={() => reqOtpMutation.mutate({ email: email! })} className='text-blue-500 cursor-pointer'>
                                                Resend OTP
                                            </button>
                                        ) : (
                        ...
                        step === "reset" && (
                            <>
                                ...
                                <form onSubmit={handleSubmit(onSubmitPassword)} >
                                    ...
                                    <input type="password" className="w-full p-2 border border-gray-300 outlie-o rounded mb-1" {
                                        ...register("password", {
                                    ...
                                    <button type="submit" className='w-full mt-4 text-lg cursor-pointer bg-black text-white py-2 rounded-lg' disabled={resetPasswordMutation.isPending} >
                                        {
